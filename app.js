                ...
                switch (parsedCommand.command) {
                    ...
                    case "host": // done
                        ...
                        getRankFromSteamId(user.steam).then(rank => {
                            ...
                            if (rank < rankRequirement) {
                                reply(message, "You are not high enough rank to host this lobby. (Your rank: `" + getRankString(rank) + "`, required rank: `" + getRankString(rankRequirement) + "`)");
                                return 0;
                            }
                            // good to start
                            ...
                            reply(message, leagueChannels[leagueRole] + " " + regionTags[region] + " Lobby started by <@" + user.discord + "> `" + getRankString(rank) + "`. Type `!cb join " + region.toLowerCase() + "` to join! **[**`" + getRankString(lobbies[leagueRole][user.steam]["rankRequirement"]) + "` **required to join]** The bot will whisper you the password on Discord. Please do not post it here.`(" + currentLobby.players.length + "/8)`", false, false);
                        ...
                        if (parsedCommand.args.length > 0) { // TODO: DRY
                            ...
                            User.findAll({where: {[Op.or]: wheres}}).then(players => {
                                getSteamPersonaNames(lobby.players).then(personas => {
                                    ...
                                    delete lobbies[leagueRole][hostLobbyStart.region];
                    ...
                    case "join": // done
                        ...
                        if (parsedCommand.args.length === 0) {
                            reply(message, "Need to specify host to join."); // TODO: Change to available lobbies.
                            return 0;
                        }
                        if (validRegions.includes(parsedCommand.args[0].toUpperCase())) {
                            reply(message, "Invalid argument. Use `!cb join [@discord]`.");
                            return 0;
                        }

                        let hostDiscordId = parseDiscordId(parsedCommand.args[0]);

                        User.findOne({where: {discord: hostDiscordId}}).then(function (hostUser) {
                            if (hostUser === null) {
                            ...
                            getRankFromSteamId(user.steam).then(rank => {
                                if (rank < leagueRequirements[leagueRole]) {
                                    reply(message, "You are not high enough rank to join lobbies in this league. (Your rank: `" + getRankString(rank) + "`, required league rank: `" + getRankString(leagueRequirements[leagueRole]) + "`)");
                                    return 0;
                                }
                                if (rank < lobbies[leagueRole][hostUser.steam]["rankRequirement"]) {
                                    reply(message, "You are not high enough rank to join lobbies in this league. (Your rank: `" + getRankString(rank) + "`, required lobby rank: `" + getRankString(lobbies[leagueRole][hostUser.steam]["rankRequirement"]) + "`)");
                                    return 0;
                                }
                                ...
                                getSteamPersonaNames([user.steam]).then(personaNames => {
                                    reply(message, "<@" + message.author.id + "> \"" + personaNames[user.steam] + "\" `" + getRankString(rank) + "` joined <@" + hostUser.discord + "> " + lobbies[leagueRole][hostUser.steam]["region"] + " region lobby. `(" + lobbies[leagueRole][hostUser.steam].players.length + "/8)`", false, false);
                    ...
                    case "quit":
                        ...
                        User.findOne({where: {steam: hostDiscordQuitId}}).then(function (hostUser) {
                            ...
                            if (index > -1) {
                                lobbies[leagueRole][hostUser.steam].players.splice(index, 1);
                                getSteamPersonaNames([user.steam]).then(personaNames => {
                                    reply(message, "<@" + message.author.id + "> \"" + personaNames[user.steam] + "\" left " + playerLobbyLeave.region + " region lobby. `(" + lobbies[leagueRole][hostUser.steam].players.length + "/8)`", false, false);
                                    // message.delete("Processed").catch(console.error);
                                });
                            }
                        });
                                ...
                                if (lobby.host !== null && lobby.password !== null) {
                                    ...
                                    User.findAll({where: {[Op.or]: wheres}}).then(players => {
                                        getSteamPersonaNames(lobby.players).then(personas => {
                                            ...
                                            reply(message, "@" + lobby.region + " [`" + getRankString(lobby.rankRequirement) + "`] " + hostDiscord + " | " + playerDiscordIds.join(" | ") + " `(" + lobby.players.length + "/8)`", false, false);
                                        });
                                    });
                        ...
                        if (existingUsers.length > 1) {
                            ...
                            if (user === null) {
                                User.create({
                                    ...
                                    reply(message, "I have linked your steam id `" + steamIdLink + "`! If I did not promote you right away then you probably used the wrong steam id or you are set to Invisible on Discord.");
                    ...
                    if (parsedCommand.args.length === 1) {
                        ...
                        if (user !== null && user.steam !== null && user.steamLinkToken === null) {
                            getRankFromSteamId(user.steam).then(rank => {
                                ...
                                user.update({rank: rank});
                                updateRoles(message, user, true, false, true, true);
                            });
                        } else {
                            reply(message, "You have not linked a steam id. See <#542454956825903104> for more information.");
                        }
    ...
    if (user !== null && user.steam !== null) {
        getRankFromSteamId(user.steam).then(rank => {
            ...
            if (message.member === null) {
                ...
                if (notify) {
                    let rankStr = getRankString(rank);
                    if (added.length > 0) {
