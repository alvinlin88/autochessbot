  ...
  user
    ...
    .catch(function(error) {
      if (error.code === 50007) {
        ...
        sendChannelandMention(
          discordClient.channels.find(r => r.name === "chessbot-commands").id,
          userDiscordId,
        ...
        sendChannelandMention(
          discordClient.channels.find(r => r.name === "chessbot-warnings").id,
          userDiscordId,
...
function getRanksFromSteamIdList(steamIdList) {
  return new Promise(function(resolve, reject) {
    request(
      "http://101.200.189.65:431/dac/ranking/get?player_ids=" +
        steamIdList.join(",") +
        "&hehe=" +
...
discordClient.on("ready", () => {
  ...
  try {
    discordClient.channels
      .get(discordClient.channels.find(r => r.name === "staff-bot").id)
      .send("I am back!")
      .then(logger.info)
      .catch(logger.error);
  } catch (err) {
...
discordClient.on("message", message => {
  ...
  if (linkAliases.includes(parsedCommand.command)) {
    let reminder = `These commands are deprecated. Please follow instructions in <#${
      discordClient.channels.find(r => r.name === "readme").id
          ...
          (function() {
            ...
            getRankFromSteamId(user.steam).then(rank => {
              ...
              userPromise.then(function(hostUser) {
                ...
                if (
                  !lobbies.hasHostedLobbyInChannel(
                    leagueChannel,
                    hostUser.steam
                  )
                ) {
          ...
          (function() {
            ...
            User.findOneBySteam(hostDiscordQuitId).then(function(hostUser) {
              if (
                ...
                getSteamPersonaNames([user.steam]).then(personaNames => {
                  let numPlayersLeft = lobbies.getLobbyForHost(
                    leagueChannel,
                    hostUser.steam
                  ).players.length;
                  sendChannel(
              ...
              if (
                ...
                sendChannelandMention(
                  ...
                  "kicked " +
                    ...
                    hostLobby.region +
                    " region lobby. `(" +
                    lobbies.getLobbyForHost(leagueChannel, user.steam).players
                      .length +
                    "/8)`"
                );
                sendDM(
            ...
            for (let hostId in lobbiesInLeagueChannel) {
              if (lobbiesInLeagueChannel.hasOwnProperty(hostId)) {
                ...
                if (lobby.host !== null && lobby.password !== null) {
                  User.findAllUsersWithSteamIdsIn(lobby.players).then(
                    players => {
                      getSteamPersonaNames(lobby.players).then(personas => {
                        let playerDiscordIds = [];
        ...
        case "lobby":
          (function() {
            ...
            User.findByDiscord(lobbyHostDiscordId).then(hostUser => {
              let lobby = lobbies.getLobbyForPlayer(
                leagueChannel,
                hostUser.steam
              );
        ...
        case "sendpass":
          (function() {
            ...
            User.findOneBySteam(playerSendPassLobby.host).then(function(
              ...
              let lobby = lobbies.getLobbyForHost(
                leagueChannel,
                hostUser.steam
              );
              sendDM(
    ...
    switch (parsedCommand.command) {
        ...
        (function() {
          ...
          if (user !== null && user.steam !== null) {
            ...
            leagueRoles.forEach(leagueRole => {
              ...
              let roleObj = message.guild.roles.find(
                r => r.name === leagueRole
              );
      ...
      case "gets":
        (function() {
          ...
          User.findByDiscord(infoPlayerDiscordId).then(function(
            infoPlayerUser
          ) {
                  ...
                  let verificationStatus =
                    getRankUser.validated === true
                      ...
                      : `[❌ Follow instructions in <#${
                          discordClient.channels.find(r => r.name === "readme")
                            .id
                        }> to verify] `;
